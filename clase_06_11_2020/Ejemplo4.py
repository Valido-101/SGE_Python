...
for x in palabras:
    if(diccionario.__contains__(x)==False):
        diccionario[x]=palabras.count(x)
        if(diccionario[x]>maximo):
            maximo=diccionario[x]
            palabra_mas_repetida="La palabra que mas se repite es '"+x+"' y aparece "+str(diccionario[x])+" veces."
